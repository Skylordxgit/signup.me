...
import { ArrowUpRight, Building2, LayoutDashboard, LogOut, Palette, Power, RefreshCw, ShieldCheck, Users, UserPlus, X } from "lucide-react";
...
type MasterTab = 'overview' | 'workspaces' | 'users' | 'branding' | 'signup';
  ...
  return <div className="admShell admShellPlain">
    <div className="admWorkspace">
      <header className="admTopbar">
        <div className="admTopbarTitle"><div><span>Platform Administration</span><h1>Master Admin Panel</h1></div></div>
        <div className="admHeaderActions">
          <span className="admBadge"><ShieldCheck size={13} />{email}</span>
          <IconButton icon={RefreshCw} label="Refresh data" disabled={busy} onClick={() => void run(async () => {})} />
      ...
      <main className="admMain" aria-busy={busy || loading}>
        ...
        <nav className="admTabNav" aria-label="Master sections">
          <button type="button" className={`dmTabButton `} onClick={() => setActiveTab('overview')}><LayoutDashboard size={16} />Overview</button>
          <button type="button" className={`dmTabButton `} onClick={() => setActiveTab('workspaces')}><Building2 size={16} />Workspaces ({workspaces.length})</button>
          <button type="button" className={`dmTabButton `} onClick={() => setActiveTab('users')}><Users size={16} />All Users ({users.length})</button>
          <button type="button" className={`dmTabButton `} onClick={() => setActiveTab('branding')}><Palette size={16} />Global Branding</button>
          <button type="button" className={`dmTabButton `} onClick={() => setActiveTab('signup')}><UserPlus size={16} />Signup Access</button>
        </nav>

        {loading ? <EmptyState title="Loading Master Admin data..." /> : <>
          {activeTab === 'overview' && <div className="admSettingsStack">
            <div className="admMetrics">
              <article className="admMetric"><div><span>Total Workspaces</span><strong>{workspaces.length}</strong></div></article>
              <article className="admMetric"><div><span>Total Pages</span><strong>{totals.pages}</strong></div></article>
              <article className="admMetric"><div><span>Total Admins</span><strong>{totals.admins}</strong></div></article>
              <article className="admMetric"><div><span>Push Subscribers</span><strong>{totals.subscribers}</strong></div></article>
            </div>

            <SectionHeading title="Recent Workspaces" />
            <div className="admMasterList">
              {workspaces.slice(0, 5).map(workspace => <article key={workspace.id} className="admMasterRow">
                ...
                <span className={`dmBadge admBadge-`}>{workspace.status}</span>
                ...
                <div className="admActionRow">
                  <IconButton icon={ArrowUpRight} label={`Manage `} disabled={busy} onClick={() => void run(() => openWorkspace(workspace.id))} />
                </div>
              </article>)}
            </div>
          </div>}

          {activeTab === 'workspaces' && <div className="admSettingsStack">
            <SectionHeading title="Workspaces Management" />
            {!workspaces.length ? <EmptyState title="No workspaces yet" /> : <div className="admMasterList">
              {workspaces.map(workspace => <article key={workspace.id} className="admMasterRow">
                ...
                <div className="admActionRow">
                  ...
                  <IconButton
                    icon={Power}
                    label={`${workspace.status === "active" ? "Disable" : "Enable"} `}
                    disabled={busy}
          ...
          {activeTab === 'users' && <div className="admSettingsStack">
            <SectionHeading title="Registered Platform Users" />
            {!users.length ? <EmptyState title="No registered users found" /> : <div className="admTeamList">{users.map(user => <article className="admTeamRow" key={user.id}>
              ...
              <IconButton icon={ArrowUpRight} label={`Manage workspace for `} onClick={() => void run(() => openWorkspace(user.workspaceId))} />
            </article>)}</div>}
          </div>}

          {activeTab === 'branding' && <div className="admSettingsStack">
            <SectionHeading title="Platform Branding" />
          ...
          {activeTab === 'signup' && <div className="admSettingsStack">
            <SectionHeading title="Public Signup Access" />
            <section className="admBrandingPanel">
